"""Build eight elbow/knee A/B influences as one Maya transaction."""
from __future__ import annotations

from typing import Mapping, Protocol

from adv_py.core.body_skeleton import BodySkeletonSnapshot
from adv_py.core.bend_volume_deform import plan_bend_volume_influences
from adv_py.core.sdk_volume_deform import SdkVolumeSpec


class BendVolumeHost(Protocol):
...
class BuildBendVolumeDeform:
    def __init__(self, host: BendVolumeHost):
        self._host = host

    def apply(self, guide: Mapping[str, object], root: str = "Root_M") -> tuple[SdkVolumeSpec, ...]:
        specs = plan_bend_volume_influences(self._host.capture_body_skeleton(root), guide)
        for spec in specs:
            ...
            for name in (spec.name, "AdvPy_VolumeBase_" + spec.name,
                ...
                if self._host.find_name_collisions(name):
                    raise ValueError("肘膝体积关节节点名称冲突：" + name)
            for name in spec.guide["sdk_sources"]:
                if self._host.find_name_collisions("AdvPy_" + name):
                    raise ValueError("肘膝体积驱动节点名称冲突：" + name)
        with self._host.transaction("构建肘膝 A/B 体积关节"):
            for spec in specs:
            ...
            for spec in specs:
                ...
                if path != spec.path or parent != spec.parent:
                    raise RuntimeError("肘膝体积关节写后复检失败：" + spec.name)
        return specs
