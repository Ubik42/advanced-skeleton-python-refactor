...
class BodyLegFkMechanismHost(FitSymmetryHost, Protocol):
...
@dataclass(frozen=True, slots=True)
class BodyLegFkMechanismBuildResult:
    plan: BodyLegFkMechanismBuildPlan
    snapshot: BodyLegFkControlSnapshot


class BuildBodyLegFkMechanismControls:
    """Create FK controls that drive validated bilateral leg FK mechanisms."""

    def __init__(self, host: BodyLegFkMechanismHost) -> None:
        self._host = host
    ...
    def plan(
        ...
        *,
        ...
        controls = plan_body_leg_fk_controls(
            body,
        ...
        return BodyLegFkMechanismBuildPlan(
            symmetry,
            body,
            mechanisms,
            controls,
        ...
        *,
        ...
        if not plan.ready:
            raise FitSkeletonValidationError(
                "Leg FK 机制控制构建预检失败，场景未修改："
                + "；".join(plan.blockers)
            )

        with self._host.transaction("创建双腿 FK 机制控制"):
            root = self._host.create_body_control_root(plan.controls.root_name)
            if root != plan.controls.root_path:
                raise RuntimeError("Leg FK 控制根路径发生漂移")
            for spec in plan.controls.controls:
                self._host.create_body_leg_fk_control(spec)
            snapshot = self._host.capture_body_leg_fk_controls(plan.controls)
            control_issues: tuple[BodyLegControlIssue, ...] = (
                audit_body_leg_fk_controls(plan.controls, snapshot)
            )
            if control_issues:
                raise RuntimeError(
                    "Leg FK 机制控制构建后复检失败："
                    + "；".join(issue.message for issue in control_issues)
                )
            if self._host.capture_body_skeleton(body_root_name) != plan.body:
                raise RuntimeError("Leg FK 机制控制构建后复检失败：Body 发生变化")
            expected_mechanisms = plan_body_leg_mechanisms(plan.body)
            if (
                self._host.capture_body_leg_mechanisms(expected_mechanisms)
                != plan.mechanisms
            ):
                raise RuntimeError("Leg FK 机制控制构建后复检失败：机制链基准状态变化")
            container = plan.symmetry.source.hierarchy.container
            if (
                self._host.capture_fit_orientation(container)
                != plan.symmetry.source
            ):
                raise RuntimeError("Leg FK 机制控制构建后复检失败：Fit joints 被改写")
            if (
                self._host.read_fit_skeleton_settings(container)
                != plan.symmetry.settings
            ):
                raise RuntimeError("Leg FK 机制控制构建后复检失败：容器设置被改写")
        return BodyLegFkMechanismBuildResult(plan, snapshot)
