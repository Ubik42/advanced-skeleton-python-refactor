...
class MayaOriginalSpinePromotionHost(MayaSpineSkinHandoffHost):
    ...
    def plan_original_spine_promotion(self, source_namespace, target_namespace,
        ...
        if mesh != mesh_path:
            raise CharacterRegistryError('原网格路径不唯一')
        retained = {_uuid(cmds, mesh), _uuid(cmds, skin_name)}
        ...
        return OriginalSpinePromotionPlan(source_namespace, target_namespace,
            source_reg, target_reg, skin_name, mesh_path, state, boundary,
    ...
    def apply_original_spine_promotion(self, plan):
        ...
        current = self.plan_original_spine_promotion(
            plan.source_namespace, plan.target_namespace,
            plan.skin_name, plan.mesh_path)
        if current != plan:
        ...
        if (self.capture_skin_handoff_boundary(plan.skin_name, plan.mesh_path)
                != plan.boundary):
            raise RuntimeError('原位接管改变原网格或变形历史 UUID')
        state = self.capture_all_skin_weights(plan.skin_name, plan.mesh_path)
        expected = CharacterIdentity(plan.source_namespace)
        if (state.vertex_count != plan.skin_state.vertex_count
