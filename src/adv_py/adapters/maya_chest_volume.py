"""Recreate original chest volume SDK graphs against rebuilt Body joints."""
from __future__ import annotations

from adv_py.core.chest_volume_deform import ChestVolumeSpec
from .maya_body import MayaBodyBuildHost


class MayaChestVolumeHost(MayaBodyBuildHost):
    def _build_graph(self, spec: ChestVolumeSpec, sdk: str) -> None:
        c = self._cmds
        ...
        def resolve(plug: str) -> str:
            ...
            if node == "Scapula_" + spec.side:
                paths = c.ls(node, long=True, type="joint") or []
                if len(paths) != 1:
                    raise RuntimeError("肩胛驱动关节缺失：" + node)
    ...
    def create_chest_volume_joint(self, spec: ChestVolumeSpec) -> None:
        ...
        if len(scale_drivers) != 1 or scale_drivers[0]["type"] != "multiplyDivide":
            raise RuntimeError("胸部体积关节缺少原版缩放混合图：" + spec.name)
        ...
        if dict(scale_row["connections"]) != {
            ...
            raise RuntimeError("胸部体积缩放输入与原版不符：" + spec.name)
        ...
        c.setAttr(joint + ".advPyAuxiliaryInfluenceKind",
                  "chest-volume-v1", type="string", lock=True)
