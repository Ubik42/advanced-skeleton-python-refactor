...
- `ExportBodyHandPose` 把双侧 14 个聚合值与 30 个 FK control 本地旋转写入 DCC 无关的 schema v1 JSON；文件不保存 Maya DAG 路径，带 SHA-256 内容摘要，使用同目录临时文件复检后原子落盘并拒绝覆盖。`ImportBodyHandPose` 先核对本工程 70 关节 Body、Hand/Pose 网络、属性范围以及全部通道的可写性和输入连接，再用一次 Undo 只恢复变化项；重复导入不创建空事务。
