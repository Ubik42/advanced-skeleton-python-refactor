...
- 当前 Maya FitSkeleton schema v1 已支持路径无关、带摘要的只读导出，向轴配置一致的现有空容器进行原子导入，以及向兼容的既有 Fit 安全增量合并；18 关节身体源增加 20 个手指关节时，全部既有关节路径与 UUID 保持不变。冲突覆盖、重父级、删除式同步、自动创建容器、改写轴配置、迁移场景对象列表或 ReBuild 脚本仍不在当前边界。
