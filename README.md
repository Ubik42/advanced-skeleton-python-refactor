...
- 当前 Maya FitSkeleton schema v1 已支持路径无关、带摘要的只读导出，以及向轴配置一致的现有空容器进行原子导入；38 关节完整五指源已验证设置、标签、元数据、位置与行为朝向往返。合并现有 Fit、自动创建容器、改写轴配置、迁移场景对象列表或 ReBuild 脚本仍不在当前边界。
