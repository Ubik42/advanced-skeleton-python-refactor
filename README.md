...
- 当前 Maya 主线已有 Arm/Leg/Hand/Global 的完整 Character 入口：双臂、双腿 FK/IK/Foot、完整五指 Hand FK 与 curl/spread 聚合层在同一事务构建，统一 Global control 驱动 Body 和九个根级目标，Hand roots 作为 Wrist 子级跟随 Arm FK/IK。30 关节旧 Body 不强制 Hand，70 关节五指 Body 自动启用 Hand，残缺五指会在事务前停止。Hand Pose schema v1 已可在 Maya 中保存、静态恢复、单侧恢复、当前帧安全写键或场景内左右镜像，并能在显式指定的 namespace 角色实例间复用；命名预设库已支持安全中文名称、原子保存、只读清单和按名应用。`BuildBodyRootMotion` 为 owned Body 创建根级输出 joint，只传递平面位移和 Up 轴旋转；`BuildBodyExportSkeleton` 在其下创建完整 owned Body 输出层级；`BakeBodyExportSkeleton` 在同一帧范围内物化 Root Motion 与每个 export joint 的 TRS，并移除全部 Body 依赖；`ExportBodyFbx` 只接受通过结构审计的独立 bake，明确选择 31 个 joint；写文件前在单一临时 Undo 事务中去掉 `AdvPy_EXP_`、角色 namespace 与内部 provenance/bake 属性，随后完整恢复原场景，再以同目录临时文件验证格式、大小和 SHA-256 后无覆盖发布。默认 Profile 保持 Maya 场景 Y/Z Up 与 cm/m 单位并写 FBX2020 binary，也可显式请求 FBX2018/2020、Y/Z Up、cm/m 和 binary/ASCII；应用层复检 exporter 回读与文件头版本。持久 export objectSet、曲线简化、更多单位、引擎骨名表、非均匀缩放与 Blender 迁移仍待后续 Maya 阶段。
