...
- `CreateAndImportFitSkeleton` 把新容器、21 项完整设置和文档关节树放入同一个 Undo Chunk；计划先检查文件摘要、场景 Up Axis、默认轴配置、容器名和全部关节短名冲突，执行后同时复检容器半径、设置与完整语义文档，一次 Undo 删除本次创建的全部场景内容但保留输入文件。
...
- 当前 Maya FitSkeleton schema v1 已支持路径无关、带摘要的只读导出，向轴配置一致的现有空容器进行原子导入，在空场景一次创建默认轴配置容器并完整恢复，以及向兼容的既有 Fit 安全增量合并。38 关节新建导入会恢复 21 项完整设置；18 关节身体源增加 20 个手指关节时，全部既有关节路径与 UUID 保持不变。冲突覆盖、重父级、删除式同步、新建容器非默认轴配置恢复、迁移场景对象列表或 ReBuild 脚本仍不在当前边界。
