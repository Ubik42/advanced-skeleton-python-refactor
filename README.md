...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本完成二十七个 Maya-only 切片：FitSkeleton 数据与朝向、M/R/L 对称展开、30 关节 Body 构建与安全 ReBuild，以及双臂 FK/IK 机制链。FK controls 驱动 FK chain；Wrist/Pole Vector controls 和 RP IK Handle 驱动 IK chain。当前尚未把两套机制结果混合到 Body，也未实现腿/手指控制、蒙皮或产品 UI。
...
- 当前 Maya 主线已覆盖双臂 FK 与 RP IK 两套独立可操作机制链；下一核心步骤是 FK/IK 双源约束、blend 属性和 Body 输出。引用场景/文件状态策略、World Match、腿/手指控制和其他 Body Build 阶段仍待后续切片。
