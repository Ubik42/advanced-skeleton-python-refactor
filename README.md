...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本完成八十三个 Maya-only 切片：FitSkeleton 数据、朝向、路径无关文档往返、安全增量合并、空场景新建导入与非默认轴配置恢复，M/R/L 对称展开，基础 30 关节与五指 70 关节 Body 构建、安全 ReBuild、双臂完整 FK/IK、双腿原子 FK/IK/Foot Rig、双手分层 FK controls、Hand curl/spread 聚合姿态、语义 Hand Pose 保存/加载、命名预设库、统一角色总控、游戏导出 Root Motion、动画 bake 与完整实时 Export Skeleton。FitSkeleton schema v1 以 `.fit.json` 保存 Up Axis、轴配置、可移植安全设置、短名拓扑、局部位置、标签、行为世界轴及受支持元数据，不携带 Maya DAG 路径、场景对象列表或 ReBuild 脚本文本；导出只读且原子落盘，完整导入既可落到轴配置一致的现有空容器，也可在空场景一次创建容器、六方向 Primary/Secondary enum、World Match、21 项设置与完整关节树，增量合并则要求全部共享关节语义一致并只创建缺失分支。所有写入路径都在一个 Undo 事务内完成恢复与全量复检。五指源模板在 Right Wrist 下定义 Thumb/Index/Middle/Ring/Pinky 各四个 Fit joints，38 个源 joints 展开成 70 个 Body joints。`BuildBodyHandFkControls` 在 Wrist_R/L 下分别建立控制根，每根手指由 1→2→3 三层曲线控制，并在绑定 offset 与 FK control 之间加入零通道 Pose 层；每侧 `handCurl`、五个单指 curl 与 `handSpread` 通过显式 DG 网络驱动 Pose 层，手动 FK rotate 仍可叠加。`ExportBodyHandPose`/`ImportBodyHandPose` 用不含 Maya 路径的 schema v1 保存这 14 个聚合值和 30 个 FK 旋转，调用方可通过 `RigA:Root_M` 这类明确角色根在多个 Maya namespace 实例间迁移同一姿态。导出允许只读捕获动画状态；导入默认保持严格静态写入，也可显式使用当前帧写键模式，并能只恢复明确指定的左手或右手。`MirrorBodyHandPose` 在场景内把一侧当前姿态镜像到对侧，保持聚合语义并按行为坐标系转换 FK 旋转。`SaveBodyHandPosePreset`、`InspectBodyHandPosePresetLibrary` 与 `ApplyBodyHandPosePreset` 再用安全中文名称和 `.handpose.json` 后缀组成 DCC 无关的目录预设库；保存原子落盘且不覆盖，清单只读校验全部文件，应用支持规范名称查询与单侧目标。只有目标侧需要满足写入条件，源侧与非目标侧仍做完整结构与结果复检。`BuildBodyCharacterRig` 保持 30 关节 Body 的 Arm/Leg/Global 行为，并在完整 70 关节五指 Body 上把 Hand 纳入同一个顶层事务。Skin 已覆盖显式绑定、稀疏权重写入、JSON 往返、路径映射、显式镜像及严格几何配对。当前尚未包含 FitSkeleton 冲突覆盖、重父级、删除式同步、场景对象列表与 ReBuild 脚本文本，也未包含整棵 Export Skeleton bake、FBX 写入、曲线简化、自动手指权重和产品 UI。
...
- `ExportFitSkeleton`/`ImportFitSkeleton` 提供带 SHA-256 摘要的 DCC 无关 schema v1：只读导出拒绝覆盖并原子落盘；导入在修改场景前完成文件、目标空容器、设置通道、轴配置和全局重名检查，随后按父级顺序恢复 38 关节示例的设置、标签、元数据与行为世界轴，并以一次 Undo 完整回到空容器。
- `CreateAndImportFitSkeleton` 把新容器、文档轴配置、21 项完整设置和关节树放入同一个 Undo Chunk；Maya adapter 使用 `X/Y/Z/-X/-Y/-Z` enum 与独立 World Match bool 保存非默认配置。计划先检查文件摘要、场景 Up Axis、容器名和全部关节短名冲突，执行后同时复检容器半径、轴配置、设置与完整语义文档，一次 Undo 删除本次创建的全部场景内容但保留输入文件。
- `MergeFitSkeleton` 对当前场景文档与输入文档做 DCC 无关的安全联合：Up Axis、轴配置、可移植设置和全部共享关节必须一致，只为缺失关节检查全局重名并按父级顺序创建；既有关节的路径与身份不变，重复合并不打开空事务，一次 Undo 只删除新增分支。
...
- 当前 Maya FitSkeleton schema v1 已支持路径无关、带摘要的只读导出，向轴配置一致的现有空容器进行原子导入，在空场景一次创建容器并恢复默认或非默认轴配置，以及向兼容的既有 Fit 安全增量合并。38 关节新建导入会恢复 21 项完整设置；18 关节身体源增加 20 个手指关节时，全部既有关节路径与 UUID 保持不变。冲突覆盖、重父级、删除式同步、迁移场景对象列表或 ReBuild 脚本仍不在当前边界。
- 当前 Maya 主线已有 Arm/Leg/Hand/Global 的完整 Character 入口：双臂、双腿 FK/IK/Foot、完整五指 Hand FK 与 curl/spread 聚合层在同一事务构建，统一 Global control 驱动 Body 和九个根级目标，Hand roots 作为 Wrist 子级跟随 Arm FK/IK。30 关节旧 Body 不强制 Hand，70 关节五指 Body 自动启用 Hand，残缺五指会在事务前停止。Hand Pose schema v1 已可在 Maya 中保存、静态恢复、单侧恢复、当前帧安全写键或场景内左右镜像，并能在显式指定的 namespace 角色实例间复用；命名预设库已支持安全中文名称、原子保存、只读清单和按名应用。`BuildBodyRootMotion` 为 owned Body 创建根级输出 joint，只传递平面位移和 Up 轴旋转，且不改变 Body 层级；`BakeBodyRootMotion` 将显式帧范围物化为三条 linear 动画曲线并移除实时约束；`BuildBodyExportSkeleton` 在 Root Motion 下创建完整 owned Body 输出层级并实时保持世界姿态。整棵 Export Skeleton bake、FBX 写入、曲线简化、自动角色发现、非均匀缩放与 Blender 迁移仍待后续 Maya 阶段。
