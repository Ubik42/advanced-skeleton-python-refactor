...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本在既有可移植骨架基线上，完成十个 Maya-only FitSkeleton 切片：关节标签、可选元数据审计、声明式元数据变更、层级构建前校验、容器设置补齐、非破坏性容器创建、自生成 Root/Spine 基础模板、声明式位置编辑、简单单子链朝向，以及 worldOrient 朝向策略预检；尚未实现完整身体 FitSkeleton、蒙皮或产品 UI。
...
- 当前 Maya FitSkeleton 已覆盖非破坏性容器与基础 Root/Spine 创建、标签、常用可选元数据、层级构建前校验、容器设置补齐、本地位置编辑、简单单子链朝向和 worldOrient 策略预检；worldOrient 写入、完整身体模板、分支朝向、镜像语义和其他几何编辑仍待后续切片。
