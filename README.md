...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本完成五十五个 Maya-only 切片：FitSkeleton 数据与朝向、M/R/L 对称展开、30 关节 Body 构建与安全 ReBuild，以及双臂完整 FK/IK 路径和双腿基础 FK/IK Rig。Arm 已覆盖模式显隐、含拉伸段长的双向匹配、带全局比例补偿的 IK stretch、轴向 twist 与横向体积保持；Leg 已把 mechanisms、FK controls、Ankle/Pole Vector IK controls、旋转/位移 blend 与模式显隐组合为单事务构建，支持固定链长范围内的单侧 FK↔IK 无跳变匹配，并可在现有基础 Rig 上原子追加双侧 reverse-foot pivot 与显式 roll/bank 通道；Skin 已覆盖显式绑定、稀疏权重写入、JSON 往返、路径映射、显式镜像及严格几何配对。自动分段 footRoll、独立 Toe IK、Leg stretch、手指控制、自动权重和产品 UI 尚未实现。
...
- `BuildBodyLegMechanisms` 复用 DCC 无关的 limb mechanism 合同，为左右腿分别生成 Hip/Knee/Ankle FK 与 IK 驱动链；共 12 个 joint 在单一事务内创建并复检，失败或一次 Undo 都会完整移除。
...
- `BuildBodyLegFkMechanismControls` 复用通用 limb FK 控制规格，为左右 Hip/Knee/Ankle 建立 6 个零通道 NURBS controls；它们只约束 Leg FK drivers，IK 链与 Body 保持隔离，控制层和机制层可分两次 Undo。
...
- `BuildBodyLegBlend` 创建左右独立的 `legIkFk_R/L`、reverse 节点、6 个双源 orientConstraint 和 Knee/Ankle 的 4 个双源 pointConstraint；FK=0、IK=1、0.5 权重及左右隔离均在 Maya 2024 中验证。
...
- `BuildBodyLegRig` 在场景零修改预演后，用一个 Undo Chunk 依次创建 Leg mechanisms、FK controls、Body blend、Ankle/PV IK 与模式显隐；任一阶段失败会移除此前所有 Leg 产物，一次 Undo 可恢复干净 Body 和 ReBuild 安全状态。
- `MatchBodyLegFkToIk` 从当前 Hip/Knee/Ankle Body 姿态计算 Ankle IK 世界帧和 Pole Vector 位置；只允许目标侧处于 FK 模式且相关通道可写，提交前复查姿态漂移，单事务切换后验证三关节位置、Ankle 朝向和另一侧 blend。
- `MatchBodyLegIkToFk` 按 Hip→Knee→Ankle 父子顺序把三层 FK controls 对齐到当前 Body 世界轴，再切回 FK；当前固定链长合同不写 driver translate，提交后复检三关节位置/朝向、另一侧 blend、Fit 输入和 FK 控制本地平移。
- `BuildBodyLegFoot` 从现有基础 Leg Rig 和 Body 的 Heel、FootSideOuter/Inner、ToesEnd、Toes 标记生成双侧五级 reverse-foot pivot；Ankle IK control 暴露 `heelRoll/outerBank/innerBank/toeRoll/ballRoll`，inner bank 通过显式负号节点保持正值语义，现有 RP IK Handle 保持世界位置后挂到 Ball pivot。名称、属性、引用/锁定输入或构建后连线异常会在单事务前阻止或整单回滚。
...
- 当前 Maya 主线已形成双臂 FK control / RP IK control → global-scale compensated stretch mechanisms → rotation/translation blend → Body → axial twist/volume helpers → 显式 Skin Bind → 精确权重写入/几何镜像 → JSON 往返/路径映射的可运行路径，并完成控制显隐与包含 IK stretch 的双向匹配；双腿基础 FK/IK 已能一次事务完整构建、撤销，在固定链长内双向无跳变匹配，并可追加经真实 Maya 验证的 Heel/Outer/Inner/Toe/Ball pivot 基础层。下一步继续在 Maya 内补足 Foot/Toe 求解语义与原子整合；Leg stretch、手指、World Match 与 Blender 迁移仍待 Maya 阶段稳定后进入。
