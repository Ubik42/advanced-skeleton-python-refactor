...
- `ConnectMocapBody` 把已验证映射建成 Maya 临时驱动层：根使用 maintain-offset parent constraint，非根使用 maintain-offset orient constraint。执行前拒绝名称冲突和已有目标输入，提交后复检真实 source/target、全部输出与当前 Body 姿态；`DisconnectMocapBody` 只删除结构完全匹配的本工程约束，两项操作各占一个 Undo。
