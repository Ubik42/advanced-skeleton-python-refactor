...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本在既有可移植骨架基线上，完成三个 Maya-only FitSkeleton 切片：关节标签、可选元数据审计，以及声明式元数据变更；尚未实现完整 FitSkeleton、蒙皮或产品 UI。
...
- 当前 Maya FitSkeleton 已覆盖标签及一组常用可选元数据的读取和变更；完整层级校验、FitSkeleton 根设置与几何编辑仍待后续切片。
