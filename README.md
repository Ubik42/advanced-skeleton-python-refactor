...
- 当前 Maya 主线已覆盖非破坏性 FitSkeleton、18 关节单侧 Right 身体源、30 实例 M/R/L 展开、一次 Undo 完成的 Body 物化与镜像行为朝向、受归属/DAG/外部连接审计保护的原子 ReBuild，以及双臂 FK 控制到 FK mechanism 的完整驱动路径；引用场景/文件状态策略、World Match、Arm IK 求解与 Body 双源混合、腿/手指控制和其他 Body Build 阶段仍待后续切片。
