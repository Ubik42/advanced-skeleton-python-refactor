...
已验证环境：Windows、Maya 2024 standalone、Blender 5.2.0 LTS background、Python 3.10/3.14。当前版本完成五十三个 Maya-only 切片：FitSkeleton 数据与朝向、M/R/L 对称展开、30 关节 Body 构建与安全 ReBuild，以及双臂完整 FK/IK 路径和双腿基础 FK/IK Rig。Arm 已覆盖模式显隐、含拉伸段长的双向匹配、带全局比例补偿的 IK stretch、轴向 twist 与横向体积保持；Leg 已把 mechanisms、FK controls、Ankle/Pole Vector IK controls、旋转/位移 blend 与模式显隐组合为单事务构建，并支持单侧 FK→IK 无跳变匹配；Skin 已覆盖显式绑定、稀疏权重写入、JSON 往返、路径映射、显式镜像及严格几何配对。Foot、Leg IK→FK、手指控制、自动权重和产品 UI 尚未实现。
