...
| 自动验证 | 333 项纯 Python 回归测试；101 个 Maya / Blender 宿主 smoke 脚本 |
...
可变脊柱的身体描述、Fit 生成、全身 FK 与蒙皮验证见 [通用身体模块](docs/通用身体模块.md)。直线绑定的曲线 IK、伸展和体积保持已通过 1 / 4 / 8 段验证；通用登记、动画保存恢复与同布局重建已贯通；弯曲绑定、模式匹配和拓扑变化迁移继续推进。
