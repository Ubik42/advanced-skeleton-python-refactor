...
def main(output: Path) -> int:
    ...
    try:
        ...
        smile = cmds.duplicate(neutral, name="hero:SmileTarget")[0]
        viseme = cmds.duplicate(neutral, name="hero:VisemeATarget")[0]
        cmds.move(.5, 0., .2, smile + ".vtx[0]", relative=True)
        cmds.move(0., .4, .3, viseme + ".vtx[2]", relative=True)
