...
- IK/FK 案例覆盖最小三关节 limb，并验证 Maya 双臂控制显隐、Wrist IK 朝向、旋转/位移输出、单侧双向匹配、IK stretch 和每段两个轴向 twist helper；体积保持、全局缩放补偿及 stretch 状态下的 IK→FK 匹配尚未完成。
...
- FK controls 与 RP IK controls 已通过双源 blend 输出到 Body，并完成控制显隐、双向匹配、stretch、轴向 twist helper 分布、显式单网格 Skin Bind、稀疏顶点权重写入/镜像、JSON 往返和路径映射；尚未实现动画 bake、体积保持、自动 namespace/influence 推断、非对称空间配对或已有蒙皮迁移。
