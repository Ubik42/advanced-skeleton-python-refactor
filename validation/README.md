...
- 原子 Body 构建和 ReBuild 已形成一次 Undo 黄金路径；Arm 已覆盖完整 FK/IK 控制、切换、匹配、stretch、twist 与 volume，Leg 已把双侧 mechanisms、FK/Ankle/PV controls、旋转/位移 blend 和模式显隐组合成原子基础 Rig，支持固定链长 FK↔IK 匹配，并可追加 Heel/Outer/Inner/Toe/Ball pivot 与显式 roll/bank 通道；自动分段 footRoll、独立 Toe IK、腿部 stretch/twist、手指控制、完整变形系统或产品 UI 尚未实现，因此不是最终 Body rig。
