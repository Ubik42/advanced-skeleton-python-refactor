...
- Root Motion 案例覆盖本工程 owned Body 的 Y-Up/Z-Up 平面位移与 yaw 实时驱动、选择保持和单次 Undo；bake 案例再覆盖显式整数帧采样、三条 linear 动画曲线、约束移除、回放一致性和 Undo 恢复。尚未包含曲线简化、完整导出骨架、FBX 文件写入或游戏引擎坐标转换。
