...
- FitSkeleton I/O schema v1 已用自行生成的 38 关节五指源验证路径无关导出、摘要复检、空容器导入、设置/标签/元数据/位置/行为轴恢复、重复拒绝与一次 Undo。安全增量合并另从兼容的 18 关节基线只新增 20 个手指关节，并保持既有关节路径和 UUID。目标容器必须已经存在且具有相同 Up Axis、轴配置和可移植设置；当前不覆盖冲突、不重父级、不做删除式同步、不自动创建容器，也不迁移场景对象列表或 ReBuild 脚本文本。
