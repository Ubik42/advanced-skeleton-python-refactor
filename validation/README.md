...
- 原子 Body 构建和 ReBuild 已形成一次 Undo 黄金路径；Arm 已覆盖完整 FK/IK 控制、切换、匹配、stretch、twist 与 volume，Leg 默认主流程已把五关节 mechanisms、四层 FK controls、Ankle/PV IK、含 Toes 的旋转/位移 blend、模式显隐、Heel/Outer/Inner/Toe/Ball pivot 以及 Ankle/Toes IK 朝向输出组合成原子 Rig；独立 Toe IK control、Foot-aware FK→IK 属性求解、自动分段 footRoll、腿部 stretch/twist、手指控制、完整变形系统或产品 UI 尚未实现，因此不是最终 Body rig。
